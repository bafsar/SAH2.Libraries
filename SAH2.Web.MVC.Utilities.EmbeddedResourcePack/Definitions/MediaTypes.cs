 ...
 ***                                                                                                     ***
 ***                                                                                                     ***
 ***  Took from https://stackoverflow.com/a/10373334/2374053 address and have been made little changes   ***
 ***                                                                                                     ***
 ***                                                                                                     ***
 ***********************************************************************************************************
 ***********************************************************************************************************
...
namespace SAH2.Web.MVC.Utilities.EmbeddedResourcePack.Definitions
{
    public class MediaTypes
    {
        ...
        /// <summary>
        ///     ECMAScript/JavaScript; Defined in RFC 4329 (equivalent to application/ecmascript but with looser processing
        ///     rules) It is not accepted in IE 8 or earlier - text/javascript is accepted but it is defined as obsolete in RFC
        ///     4329. The "type" attribute of the &lt;/script&gt; tag in HTML5 is optional and in practice omitting the media type
        ///     of JavaScript programs is the most interoperable solution since all browsers have always assumed the correct
        ...
        ///     cross-browser support. The "type" attribute of the &lt;/script&gt; tag in HTML5 is optional and there is no need to
        ///     use it at all since all browsers have always assumed the correct default (even in HTML 4 where it was required by
        ///     the specification).
        /// </summary>
        //[Obsolete]
        ...
        public static string GetMediaType(string fileId)
        {
            if (fileId.EndsWith(".js"))
                return TextJavascript;

            if (fileId.EndsWith(".css") || fileId.EndsWith(".less"))
                return TextCss;

            if (fileId.EndsWith(".jpg") || fileId.EndsWith(".jpeg"))
                return ImageJpeg;

            if (fileId.EndsWith(".gif"))
                return ImageGif;

            if (fileId.EndsWith(".png"))
                return ImagePng;

            if (fileId.EndsWith(".bmp"))
                return ImageBmp;

            if (fileId.EndsWith(".tif") || fileId.EndsWith(".tiff"))
                return ImageTiff;
